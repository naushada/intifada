...
std::string HTTPClient::processRequestAndBuildResponse(const std::string& in) {
    ...
    switch(whichResponse) {
        case HTTPUriName::GetChangeEventsNotification:
        {
            ...
            std::cout << basename(__FILE__) << ":" << __LINE__ << " HTTPUriName::GetChangeEventsNotification in:\n" << std::endl << in << std::endl;
            if(handleEventsNotificationResponse(in)) {
                ...
                std::cout << basename(__FILE__) << ":" << __LINE__ << " HTTPUriName::GetChangeEventsNotification" << std::endl;
            }
        }
        break;
        case HTTPUriName::GetDataPoints:
        {
            std::cout << basename(__FILE__) << ":" << __LINE__ << " HTTPUriName::GetDataPoints in:\n" << std::endl << in << std::endl;
            if(handleGetDatapointResponse(in)) {
                ...
                /// @brief Error Handling
                std::cout << basename(__FILE__) << ":" << __LINE__ << " HTTPUriName::GetDataPoints" << std::endl;
            }
        ...
        case HTTPUriName::GetTokenForSession:
        {
            ...
            std::cout << basename(__FILE__) << ":" << __LINE__ << " HTTPUriName::GetTokenForSession in:\n" << std::endl << in << std::endl;
        ...
        {
            std::cout << basename(__FILE__) << ":" << __LINE__ << " HTTPUriName::RegisterDataPoints in:" << std::endl << in << std::endl;
            if(handleRegisterDatapointsResponse(in)) {
                sentURI(HTTPUriName::GetChangeEventsNotification);
                return(buildGetEventsNotificationRequest());
            } else {
                /// @brief Error Handling
                std::cout << basename(__FILE__) << ":" << __LINE__ << " HTTPUriName::RegisterDataPoints" << std::endl;
            }
        }
        break;
...
std::string HTTPClient::buildHeader(HTTPUriName path, const std::string& payload) {
    ...
    if(!method.compare(0, 4, "POST")) {
        ss << "Accept: application/vnd.api+json\r\n"
...
bool HTTPClient::handleRegisterDatapointsResponse(const std::string& in) {
    std::cout << basename(__FILE__) << ":" << __LINE__ << " handleRegisterDatapointsResponse:\n"<< in << std::endl;
    bool ret = false;
    Http http(in);
...
bool HTTPClient::handleSetDatapointResponse(const std::string& in) {
    std::cout << basename(__FILE__) << ":" << __LINE__ << " handleSetDatapointResponse " << std::endl;
    return(false);
}
